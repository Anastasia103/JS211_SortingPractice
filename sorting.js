...
return nums
  ...
  const sumEvens = nums.filter(x => x % 2 == 0).reduce((x,y) => x + y, 0);

  console.log(sumEvens);

  // Find the index of the first value when added to it's index = 512 (#ATX!!)
  const atxIdx = nums.findIndex((x, xIndex) => xIndex + x === 512)
  ...
  const weatherStates = weather.map(a => a.weather_state_name)
  uniqueWeatherStates= [... new Set(weatherStates)]
  console.log(uniqueWeatherStates)

  //find the id of the object in weather that has a min_temp of 15.915

  const idealTemp = weather.filter(x => x.min_temp === 15.915).shift().id
